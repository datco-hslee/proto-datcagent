...
const ChatbotTutorial: React.FC<ChatbotTutorialProps> = ({ isOpen, onClose }) => {
  ...
  const executeEnhancedNavigation = async (response: EnhancedChatbotResponse) => {
    ...
    try {
      // 기존 인스턴스 제거
      if (driverRef.current) {
        console.log('기존 Driver.js 인스턴스 제거');
        driverRef.current.destroy();
        await new Promise(resolve => setTimeout(resolve, 200));
      }
      
      // 새 인스턴스 생성
      console.log('새 Driver.js 인스턴스 생성');
      driverRef.current = createDriverInstance();
      ...
      if (!sectionElement) {
        console.error(`섹션 요소를 찾을 수 없음: ${sectionId}`);
        // 모든 섹션 요소 출력
        const allSections = document.querySelectorAll('[data-section-id]');
        console.log('사용 가능한 섹션:', Array.from(allSections).map(el => el.getAttribute('data-section-id')));
        return;
      }
      ...
      // 스텝 각각에 대해 요소 존재 확인
      for (const step of response.driverSteps) {
        try {
          const el = document.querySelector(step.element);
          console.log(`요소 확인 ${step.element}: ${el ? '존재함' : '없음'}`);
        } catch (e) {
          console.error(`선택자 오류 ${step.element}:`, e);
        }
      }
      ...
      driverRef.current.setConfig({
        ...
        onNextClick: async (_element: any, _step: any, options: any) => {
          ...
          if (currentStepIndex === 1 && response.navigationPath) {
            ...
            console.log(`메뉴 항목 검색: [data-menu="${menuId}"]`);
            
            const menuElement = document.querySelector(`[data-menu="${menuId}"]`) as HTMLElement;
            if (menuElement) {
              console.log('메뉴 항목 발견, 클릭 실행');
              menuElement.click();
              
              // 페이지 로딩 대기
              console.log('페이지 로딩 대기 (1.5초)');
              ...
              if (response.driverSteps && response.driverSteps.length > 2) {
                ...
                if (addButton) {
                  console.log('새 고객 추가 버튼 발견!', addButton);
                } else {
                  console.warn('새 고객 추가 버튼을 찾을 수 없음');
                  
                  // 모든 버튼 출력
                  const allButtons = document.querySelectorAll('button');
                  console.log(`버튼 ${allButtons.length}개 발견`);
              ...
              return driverRef.current.moveNext();
            } else {
              console.error(`메뉴 항목을 찾을 수 없음: ${menuId}`);
              // 모든 메뉴 항목 출력
              const allMenus = document.querySelectorAll('[data-menu]');
              console.log('사용 가능한 메뉴 항목:', Array.from(allMenus).map(el => el.getAttribute('data-menu')));
            }
          }
